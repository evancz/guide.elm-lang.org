...
When you look at the `view` function, you see the `onClick` attributes are set to give out `Increment` and `Decrement` values. These `Msg` values will get fed directly into our `update` function, driving our whole app forward.
